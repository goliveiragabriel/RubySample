...
set :scm, :none                     # :subversion
set :repository, "."                # "http://svn2.xp-dev.com/svn/bemcasado"
set :deploy_via, :copy              # :remote_cache
# set :user, "andreteves"           # The deploy user
# set :scm_passphrase, "andre1985"  # The deploy user's password
...
namespace :custom_task do
  before "deploy:rollback:revision", "custom_task:rollback_database"
  ...
  task :rollback_database, :roles => :db, :only => { :primary => true } do
    if releases.length < 2
      ...
      #run "cd #{current_path}; #{rake} RAILS_ENV=#{rails_env} #{migrate_env} db:migrate VERSION=20111231212317"
    end
  end

  before "deploy:finalize_update", "custom_task:copy_public_folder"
  desc "Copies the public folder from the previously deployed release"
  task :copy_public_folder, :roles => :app do
    if releases.length < 2
      abort "could not copy the public folder because there is no prior release"
    else
      run "cd /mnt/bemcasados-production/releases;"
      # blog
      run "#{sudo} cp -R #{previous_release}/public/blog/ #{current_release}/public"
      run "#{sudo} chmod -R 755 #{current_release}/public/blog"
      run "#{sudo} chmod -R 775 #{current_release}/public/blog/wp-content"
      run "#{sudo} chown -R www-data:www-data #{current_release}/public/blog"
      run "#{sudo} chmod -R 777 #{current_release}/log" # para corrigir o problema na crontab via whenever
    end
  end

  after "rubber:bootstrap", "custom_task:grant_permissions"
  task :grant_permissions, :roles => :db, :only => { :primary => true } do
    grant_sql = "GRANT ALL PRIVILEGES ON bemcasados_production.* TO ''@'db01.bemcasados.art.br';"
    run "mysql -u root -e \"#{grant_sql}\""
    # SHOW GRANTS FOR ''@'db01.bemcasados.art.br';
  end

  #after "deploy", "custom_task:update_sitemap"
  ...
  # adicionar a porta 80 (HTTP) no security group bemcasados_production_db01
  # adiconar a linha: "Alias /phpmyadmin /usr/share/phpmyadmin/" em /etc/apache2/sites-enabled/000-default
  # uncomment a linha "AllowNoPassword" em /etc/phpmyadmin/config.inc.php
