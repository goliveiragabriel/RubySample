...
(function($) { 
    var JFMFS = function(element, options) {
        ...
        var settings = $.extend({
            max_selected: -1,
            max_selected_message: "{0} de {1} selecionados",
			pre_selected_friends: [],
			exclude_friends: [],
			friend_fields: "id,name,gender,relationship_status,installed",
			sorter: function(a, b) {
			...
			labels: {
				...
				filter_default: "",
				filter_title: "Encontre seus amigos",
				all: "Todos",
				max_selected_message: "{0} de {1} selecionados"
			}
        }, options || {});
        ...
        elem.html(
            ...
            "        <a class='filter-link selected' id='send-message-fb' href='#'> Enviar convite </a>" +
...
//            "    </div>" +
				...
				if(! (friend.id in excluded_friends_graph)) {
                    if(friend['installed']){
                        ...
                        if(friend['relationship_status']){
                            if(friend['relationship_status'] == "Married" || friend['relationship_status'] == "Engaged" || friend['relationship_status'] == "In a relationship"){
                                if (friend['relationship_status'] == "Married") {
                                    if (friend['gender'] == "male"){
                                        var relationship_status = "Casado"
                                    }else{
                                        var relationship_status = "Casada"
                                    }
                                } else if (friend['relationship_status'] == "Engaged") {
                                    if (friend['gender'] == "male"){
                                        var relationship_status = "Noivo"
                                    }else{
                                        var relationship_status = "Noiva"
                                    }
                                } else {
                                    var relationship_status = "Namorando"
                                }
                                selectedClass = (friend.id in preselected_friends_graph) ? "selected" : "";
                                buffer.push("<div class='jfmfs-friend " + selectedClass + " ' id='" + friend.id  +"'><img/><div class='friend-name'>" + friend.name + "</div><div class='relationship-status'>" + relationship_status + "</div></div>");
                            }
                        }
                    }
				}
        ...
        var init = function() {
            ...
            elem.find("#jfmfs-friend-filter-text")
                ...
                .focus( function() {
                    if($.trim($(this).val()) == 'Busque pelo nome') {
                        $(this).val('');
                    }
                    })
                .blur(function() {
                    if($.trim($(this).val()) == '') {
                        $(this).val('Busque pelo nome');
                    }                        
                    });
